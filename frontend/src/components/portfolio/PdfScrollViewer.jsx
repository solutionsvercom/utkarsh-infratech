...
function LazyPdfPage({ pdf, pageNumber }) {
  ...
  useEffect(() => {
    if (!visible || width <= 0 || !canvasRef.current || rendered) return undefined;
  ...
  return (
    <div ref={wrapperRef} className="mb-4 w-full flex justify-center">
      {!rendered && (
        <div className="w-full aspect-[3/4] bg-white/10 rounded-lg flex items-center justify-center">
          <Loader2 className="w-8 h-8 text-white/60 animate-spin" aria-hidden="true" />
        </div>
      )}
      <canvas
        ref={canvasRef}
        className={`block h-auto rounded-lg shadow-lg bg-white ${rendered ? '' : 'hidden'}`}
...
export default function PdfScrollViewer({ src, alt }) {
  ...
  return (
    <div
      className="w-full h-full overflow-y-auto overflow-x-auto overscroll-contain px-1 py-2"
      aria-label={alt}
    >
      {pages.map((pageNumber) => (
        <LazyPdfPage key={pageNumber} pdf={pdf} pageNumber={pageNumber} />
      ))}
    </div>
  );
}
